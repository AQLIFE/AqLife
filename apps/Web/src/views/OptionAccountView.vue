...
import { Configuration, type AccountDto, type SubscriptionFullDto } from '@/api/generated'
...
const form = reactive<AccountDto & { subscriptions: SubscriptionFullDto[] }>({
  name: '',
  desc: '',
...
onMounted(async () => {
  try {
    ...
    if (acc) {
      ...
      if (form.subscriptions != null && form.subscriptions != undefined)
        for (let i = 0; i < form.subscriptions.length; i++)
          PreviewUrls[i] =
            form.subscriptions[i].subscriptionIcon != null
              ? `${import.meta.env.VITE_API}/api/file/preview?title=&id=${form.subscriptions[i].subscriptionIcon}`
              : ''

      if (acc.avatar) {
        // 注意：如果后端返回的是 GUID，需要拼接上后端的图片查看 API 地址
        // 如果后端直接返回完整 URL，则直接赋值：
        avatarPreview.value = `${import.meta.env.VITE_API}/api/file/preview?title=&uid=${acc.avatar}`
      }
    }
  } catch {}
})
...
const saveProfile = async () => {
  try {
    // 1. 初始化请求 API
    const accountReq = new AccountApi(new Configuration(ApiOption))

    // 2. 注册基础信息
    await accountReq.apiAccountPost()

    // 3. 登录获取 Token
    // 修复原 then(result, response) 的参数错位问题
    // const loginRawResponse = await accountReq.apiAccountLoginPostRaw({
    //   loginCommand: { accountName: form.name, secretKey: SecretKey.value },
    // })
    ...
    // await accountReq.apiAccountPatch({ : form })
...
//   // const res = await fileApi.apiFileUploadPostRaw({file})
