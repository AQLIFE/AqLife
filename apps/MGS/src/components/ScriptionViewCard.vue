<template>
  <ElCard shadow="hover" @click.right.prevent="isEdit = !isEdit" @click.middle.prevent="$emit('delete', serial)">
    <template #header>
...
function preview(guid: string | null | undefined): string {
  if (typeof guid == 'string') {
    let temp = fileStore.previewUrl.get(guid)
    // console.log(temp)
    if (temp === undefined)
      temp = `${import.meta.env.VITE_API}/api/File/preview?UID=${guid}`
    return temp
  }
  return ''
}
...
.noneBorder :deep(.el-input__wrapper) {
  ...
  /* Element Plus 的边框其实是 box-shadow 实现的，必须设为 none */
