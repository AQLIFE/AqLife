...
namespace MyLife.Service.Implementations
{
    ...
    {
        Task<IEnumerable<FileMetaEntity?>> FindFileAsync(string? title, Guid? id,bool isDown=false);
        Task<IEnumerable<FileMetaEntity?>> FindAllAsync();
    }
    public class FileSearch(IEnumerable<IFileSearchStrategy> searchStrategies, AppStorage storage, IHttpContextAccessor httpContext,IOptions<FilePolicyOption> options) : IFileSearch
    {
