...
namespace MyLife.Service.Strategies
{
    ...
    public class SearchByIdStrategy : IFileSearchStrategy
    {
        ...
        public async Task<IEnumerable<FileMetaEntity?>> ExecuteAsync(IQueryable<FileMetaEntity> query, string? title, Guid? id)
    ...
    public class SearchByTitleStrategy : IFileSearchStrategy
    {
        ...
        public async Task<IEnumerable<FileMetaEntity?>> ExecuteAsync(IQueryable<FileMetaEntity> query, string? title, Guid? id)
            => await query.Where(f => f.FileName.Contains(title!)).ToListAsync();
    }
}
