...
namespace MyLife.Service.Validators.BusinessValidator
{
    ...
    public class FileTypeValidator<T>(IOptions<FilePolicyOption> options) : AbstractValidator<T> where T : IUploadRequest
    {
    ...
    /// </summary>
    /// <param name="options"></param>
    public class FileSizeValidator<T>(IOptions<FilePolicyOption> options) : AbstractValidator<T> where T : IUploadRequest
    {
    ...
    public class FileDuplicateValidator<T>(AppStorage storage, UploadContext context) : AbstractValidator<T> where T : IUploadRequest
    {
        ...
        private protected override async Task<bool> IsValidAsync(T command, CancellationToken ct)
        {
            ...
            foreach (var file in files)
            {
                ...
                if (await storage.File.AsNoTracking().AnyAsync(e => e.FileHash == tempHash)) return false;
